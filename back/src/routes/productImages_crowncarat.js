  ...
  try {
    ...
    for (const result of uploadResults) {
      const dbResult = await pool.query(
        'INSERT INTO product_images_crowncarat (product_id, image_url) VALUES ($1, $2) RETURNING *',
...
router.delete('/images/:imageId', checkAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM product_images_crowncarat WHERE id = $1 RETURNING *',
      [imageId]
    );
