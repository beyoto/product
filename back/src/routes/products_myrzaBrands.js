...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM products_myrzaBrands p
      LEFT JOIN product_images_myrzaBrands pi ON pi.product_id = p.id
...
router.get('/:id', async (req, res) => {
  try {
    ...
    pool.query('UPDATE products_myrzaBrands SET views_count = views_count + 1 WHERE id = $1', [id]);
...
router.post('/', checkAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO products_myrzaBrands (name, description, price, category)
...
router.put('/:id', checkAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE products_myrzaBrands SET 
...
router.delete('/:id', checkAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM products_myrzaBrands WHERE id = $1 RETURNING *', [id]);
