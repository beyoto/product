...
// Public list of active Etno products, with optional category and price sorting.
router.get('/', async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM products_etno p
      LEFT JOIN product_images_etno pi ON pi.product_id = p.id
...
// All products, including inactive ones, for the admin panel.
...
router.get('/:id', async (req, res) => {
  try {
    ...
    pool.query('UPDATE products_etno SET views_count = views_count + 1 WHERE id = $1', [id]);
...
router.post('/', checkAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO products_etno (name, description, price, category)
...
router.delete('/:id', checkAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM products_etno WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Product not found' });
    }
